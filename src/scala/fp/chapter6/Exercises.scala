...
class Exercises extends FunSuite {
  ...
  object State {
    ...
    test("Exercise 1. Write a function to generate a random positive integer") {
      println(RNG.positiveInt(RNG.simple(1))._1)
    }

    test("Exercise 2. Write a function to generate a Double between 0 and 1, not including 1") {
      println(RNG.double(RNG.simple(13)))
    }

    test("Write function to generate pairs") {
      println(RNG.intDouble(RNG.simple(1)))
      println(RNG.doubleInt(RNG.simple(1)))
      println(RNG.double3(RNG.simple(1)))
    }

    test("Exercise 4. Write a function to generate a list of random integers") {
      println(RNG.ints(10)(RNG.simple(13)))
    }

    test("Exercise 5. Use map to generate an Int between 0 and n, inclusive") {
      println(RNG.positiveMax(100))
    }

    test("Excercise 6. Use map to reimplement RNG.double in a more elegant way") {
      println(RNG._double)
    }

    test("Exercise 7. More powerful map. Reimplement intDouble and doubleInt from before") {
      println(RNG.randIntDouble)
    }

    test("Exercise 8. Implement sequence, for combining a List of transitions into a single transition") {
      // done
    }

    test("Exercise 9. Implement flatmap, then use it to reimplement positiveInt") {
      // done
    }

    test("Exercise 10. Reimplement map and map2 in terms of flatMap") {
      // done
    }

    test("Exercise 11. Generalize the functions unit, map, map2, flatMap and sequence. Add them as methods on the case class where possible.") {
      // done
    }

    test("Exercise 12. Come up with signatures for get and set, then write their implementations") {
      // do
    }

    test("Exercise 13. Candy dispenser") {
