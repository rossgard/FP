...
trait Stream[+A] {
  ...
  def takeWhileByFoldRight(p: A => Boolean): Stream[A] = foldRight(empty[A])((h, t) =>
    if (p(h)) cons(h, t)
    else empty)

  def mapByFoldRight[B](p: A => B): Stream[B] = foldRight(empty[B])((h, t) => cons(p(h), t))

  def filterByFoldRight(p: A => Boolean): Stream[A] = foldRight(empty[A])((h, t) =>
    if (p(h)) cons(h, t)
    else empty)
  ...
  def appendByFoldRight[B >: A](s: Stream[B]): Stream[B] = foldRight(s)((h, t) => cons(h, t))

  def flatMapByFoldRight[B](f: A => Stream[B]): Stream[B] = foldRight(empty[B])((h, t) => f(h) appendByFoldRight t)

  def forAllByFoldRight(p: A => Boolean): Boolean = foldRight(true)((a, b) => p(a) && b)
  ...
  /*
  `uncons` has exactly the correct signature needed for `unfold`.
  (Notice that for any `Stream`, `s`, `unfold(s)(_.uncons) == s`.
  To implement `Stream.map`, we simply transform the first element of the pair returned, using the `map` method on
  `Option` (discussed in chapter 4). Variation 1 has the same function written using explicit pattern matching on `Option`.
  */
  def mapViaUnfold[B](f: A => B): Stream[B] =
    unfold(this)(_.uncons.map {
      case (h, t) => (f(h), t)
    })
  ...
  /*
 Notice we are using a _pattern label_ here. In front of any pattern, `x`,
 we can say `labelName@x`, to introduce a variable we can reference on the right-hand side.
 Here we are giving `Some((h,_))` a name to allow us to simply reuse the value (rather than reconstructing it)
 if the guard (in this case `if f(h)`) passes.
 */
  ...
  /*
  There are a number of edge cases with this function.
  We can deal with some of these edge cases by treating each stream as an infinite series of `Option` values,
  which become `None` when the stream is exhausted.
  */
...
object Stream {
  ...
  */
  val fibsViaUnfold = cons(0,
    unfold((0, 1)) {
      case (f0, f1) => Some((f1, (f1, f0 + f1)))
    })

  def fromViaUnfold(n: Int) = unfold(n)(n => Some(n, n + 1))

  def constantViaUnfold(n: Int) = unfold(n)(_ => Some(n, n))

  val onesViaUnfold = unfold(1)(_ => Some(1, 1))

  /*
  s.startsWith(s2) when corresponding elements of s and s2 are all equal,
  until the point that s2 is exhausted. If s is exhausted first, or we find an element that doesn't match, we terminate early.
  Using non-strictness, we can compose these three separate logical steps - the zipping, the termination when the second stream
  is exhausted, and the termination if a nonmatching element is found or the first stream is exhausted.
  */
